...
public class SimpleFrameUI extends AbstractComponentUI<Frame> {
	...
	protected void renderComponent(Frame component) {
		...
		RenderUtil.setColor(new Color(0, 0, 0));
		glBegin(GL_QUADS);
		{
			glVertex2d(0, 0);
		...
		for(int i = 0; i < checks.length; i++) {
			if(!checks[i])
				continue;
			RenderUtil.setColor(new Color(0,128,255));//component.getBackgroundColor());
			glBegin(GL_QUADS);
			{
			...
			glEnd();
			if(overlays[i]) {
				glColor4f(0.0f, 128.0f, 255.0f, 0.1f);
				glBegin(GL_QUADS);
				{
					glVertex2d(offset - fontHeight, 2);
		 			glVertex2d(offset, 2);
