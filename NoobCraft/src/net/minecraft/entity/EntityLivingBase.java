...
public abstract class EntityLivingBase extends Entity
{
	...
	public static final String __OBFID = "CL_00001549";
	...
	protected void func_180433_a(double p_180433_1_, boolean p_180433_3_, Block p_180433_4_, BlockPos p_180433_5_)
	{
		...
		if (!this.worldObj.isRemote && this.fallDistance > 3.0F && p_180433_3_)
		{
			...
			if (var7.getMaterial() != Material.air)
			{
				...
				((WorldServer) this.worldObj).func_175739_a(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, var11, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, new int[]
				{ Block.getStateId(var6) });
			}
		}
			...
			if (!this.canBreatheUnderwater() && !this.isPotionActive(Potion.waterBreathing.id) && !var7)
			{
				...
				if (this.getAir() == -20)
				{
					...
					for (int var3 = 0; var3 < 8; ++var3)
					{
						...
						this.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double) var4, this.posY + (double) var5, this.posZ + (double) var6, this.motionX, this.motionY,
		...
		if (this.deathTime == 20)
		{
			...
			for (var1 = 0; var1 < 20; ++var1)
			{
				...
				this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width,
						this.posY + (double) (this.rand.nextFloat() * this.height), this.posZ + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, var8, var4, var6,
						new int[0]);
			}
		}
	}
	...
	protected void updatePotionEffects()
	{
		...
		if (var11 > 0)
		{
			...
			if (var4 && var11 > 0)
			{
				...
				this.worldObj.spawnParticle(var12 ? EnumParticleTypes.SPELL_MOB_AMBIENT : EnumParticleTypes.SPELL_MOB, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY
						+ this.rand.nextDouble() * (double) this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, var5, var7, var9, new int[0]);
			}
		}
	}
	...
	public void renderBrokenItemStack(ItemStack p_70669_1_)
	{
		...
		for (int var2 = 0; var2 < 5; ++var2)
		{
			...
			this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, var6.xCoord, var6.yCoord, var6.zCoord, var3.xCoord, var3.yCoord + 0.05D, var3.zCoord, new int[]
			{ Item.getIdFromItem(p_70669_1_.getItem()) });
		}
	}
	...
	{
		return (EntityLivingBase) (this._combatTracker.func_94550_c() != null ? this._combatTracker.func_94550_c() : (this.attackingPlayer != null ? this.attackingPlayer
	...
	{
		return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this
				.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6);
	}
			...
			for (int var11 = -var9; var11 < var9; ++var11)
			{
				if (var10 != 0 || var11 != 0)
				{
					...
					if (this.worldObj.func_147461_a(var2).isEmpty())
					{
						...
						if (World.doesBlockHaveSolidTopSurface(this.worldObj, new BlockPos(var12, (int) this.posY - 1, var13))
								|| this.worldObj.getBlockState(new BlockPos(var12, (int) this.posY - 1, var13)).getBlock().getMaterial() == Material.water)
						{
	...
	public void moveEntityWithHeading(float p_70612_1_, float p_70612_2_)
	{
		...
		if (this.isServerWorld())
		{
			...
			} else
			{
				...
				if (this.onGround)
				{
					var3 = this.worldObj.getBlockState(
							new BlockPos(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.getEntityBoundingBox().minY) - 1, MathHelper.floor_double(this.posZ))).getBlock().slipperiness * 0.91F;
				}
				...
				if (this.worldObj.isRemote
						&& (!this.worldObj.isBlockLoaded(new BlockPos((int) this.posX, 0, (int) this.posZ)) || !this.worldObj
								.getChunkFromBlockCoords(new BlockPos((int) this.posX, 0, (int) this.posZ)).isLoaded()))
				{
					if (this.posY > 0.0D)
					{
						this.motionY = -0.1D;
	...
	public void onLivingUpdate()
	{
		...
		if (this.isJumping)
		{
			...
			} else if (this.onGround && this.jumpTicks == 0)
			{
				this.jump();
				this.jumpTicks = 10;
			} else if (this instanceof EntityPlayer && Client.getInstance().getHacks().byName("Jetpack").isEnabled())
			{
				this.jump();
			}
		} else
	...
	{
		return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ), new Vec3(p_70685_1_.posX, p_70685_1_.posY + (double) p_70685_1_.getEyeHeight(),
