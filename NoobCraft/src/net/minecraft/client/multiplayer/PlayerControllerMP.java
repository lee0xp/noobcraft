...
public class PlayerControllerMP
{
	...
	{
		//    	flipPlayer(Minecraft.getMinecraft().thePlayer);
			...
			} else
			{
				...
				if (this.stepSoundTickCounter % 4.0F == 0.0F)
				{
					this.mc.getSoundHandler().playSound(
							new PositionedSoundRecord(new ResourceLocation(var3.stepSound.getStepSound()), (var3.stepSound.getVolume() + 1.0F) / 8.0F, var3.stepSound.getFrequency() * 0.5F,
									(float) p_180512_1_.getX() + 0.5F, (float) p_180512_1_.getY() + 0.5F, (float) p_180512_1_.getZ() + 0.5F));
				}
	...
	private boolean func_178893_a(BlockPos p_178893_1_)
	{
		...
		{
			var3 = var2.getItem() == this.currentItemHittingBlock.getItem() && ItemStack.areItemStackTagsEqual(var2, this.currentItemHittingBlock)
	...
	public void attackEntity(EntityPlayer playerIn, Entity targetEntity)
	{
		...
		/*
		 * Calls the crits hack
		 */
		if (Client.getInstance().getHacks().crits.isEnabled())
			Client.getInstance().getHacks().crits.onCall();
		/*
		 * End of hacks...
		 */
	 ...
	 */
	public ItemStack windowClick(int windowId, int slotId, int p_78753_3_, int tmp1, EntityPlayer playerIn)
	{
		short var6 = playerIn.openContainer.getNextTransactionID(playerIn.inventory);
		ItemStack var7 = playerIn.openContainer.slotClick(slotId, p_78753_3_, tmp1, playerIn);
		this.netClientHandler.addToSendQueue(new C0EPacketClickWindow(windowId, slotId, p_78753_3_, tmp1, var7, var6));
		return var7;
	}
